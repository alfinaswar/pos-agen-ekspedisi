...
@section('content')
    ...
    <div class="container-fluid">
        ...
        @if (isset($TenantAkanHabis) && $TenantAkanHabis->isNotEmpty())
            <div class="alert alert-warning alert-dismissible fade show shadow-sm border-0 mb-4" role="alert"
                style="background: linear-gradient(135deg, #fff3cd 0%, #ffe69c 100%); border-left: 5px solid #ffc107 !important;">
                <div class="d-flex align-items-start">
                    <i class="ti ti-alert-triangle fs-1 me-3 mt-1 text-warning-emphasis"></i>
                    <div class="flex-grow-1">
                        <h5 class="alert-heading fw-bold text-warning-emphasis mb-2">
                            Peringatan Jatuh Tempo Subscription
                        </h5>
                        <p class="mb-2 text-dark">
                            Terdapat <strong>{{ $TenantAkanHabis->count() }} Tenant</strong> yang masa berlakunya akan habis
                            dalam 7 hari ke depan. Segera buatkan tagihan perpanjangan untuk menghindari gangguan layanan.
                        </p>

                        <ul class="mb-3 small text-dark">
                            @foreach ($TenantAkanHabis as $Tenant)
                                @php
                                    $SisaHari = \Carbon\Carbon::now()->diffInDays(
                                        $Tenant->TanggalAkhirSubscription,
                                        false,
                                    );
                                    // Convert to Carbon instance if it's a string
                                    $TanggalAkhir =
                                        $Tenant->TanggalAkhirSubscription instanceof \Carbon\Carbon
                                            ? $Tenant->TanggalAkhirSubscription
                                            : \Carbon\Carbon::parse($Tenant->TanggalAkhirSubscription);
                                @endphp
                                <li class="mb-1">
                                    <strong>{{ $Tenant->Nama }}</strong>
                                    <span class="text-muted">(Habis:
                                        {{ $TanggalAkhir->format('d M Y') }})</span>
                                    <span class="badge bg-danger text-white ms-1">Sisa {{ $SisaHari }} Hari</span>
                                </li>
                            @endforeach
                        </ul>
                        ...
                        <a href="{{ route('tagihan-pembayaran.create') }}"
                            ...
                            <i class="ti ti-plus me-1"></i> Buat Tagihan Perpanjangan
                        </a>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row g-2 mb-3 align-items-end">
                            ...
                            <div class="col-md-{{ auth()->user()->role === 'Superadmin' ? '3' : '6' }}">
                                <div class="d-flex gap-2">
                                    ...
                                    <button type="button" id="BtnReset" class="btn btn-secondary btn-sm px-3"
                                        title="Reset Filter">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered dt-responsive nowrap align-middle mb-0"
                                id="TagihanTable" style="width: 100%;">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th class="text-center">Berlaku Hingga</th> <!-- Tambahan BerlakuHingga -->
                ...
                <div class="modal-content border-0 shadow">
                    ...
                    <div class="modal-body p-4">
                        <div class="alert alert-info d-flex align-items-center mb-3">
                            ...
                            <div>
                                <strong>Tagihan Terpilih:</strong> <span id="ModalSelectedCount"
                                    class="text-dark fw-bold">0</span> tagihan akan diverifikasi
                            </div>
                        </div>
                        ...
                        <form id="FormBulkVerifikasi">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-text text-muted">
                                    <i class="ti ti-info-circle me-1"></i>Status yang akan diterapkan ke semua tagihan
                                    terpilih
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="BulkCatatanVerifikasi" name="CatatanVerifikasi" rows="3"
                                    placeholder="Masukkan catatan verifikasi (opsional)..."></textarea>
                                <div class="form-text text-muted">
                                    <i class="ti ti-info-circle me-1"></i>Catatan ini akan disimpan untuk semua tagihan
                                    yang diverifikasi
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="datetime-local" class="form-control" id="BulkVerifPada" name="VerifPada"
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="BulkVerifOleh" name="VerifOleh"
                            ...
                            <div class="alert alert-warning d-flex align-items-center mb-0">
                                ...
                                <div>
                                    <strong>Perhatian!</strong> Tindakan ini akan mengubah status dan menambahkan catatan
                                    verifikasi ke semua tagihan terpilih.
                                </div>
                            </div>
                        </form>
                    </div>
                    <div class="modal-footer border-top-0 pt-0">
                        ...
                        <button type="button" class="btn btn-success text-white fw-semibold px-4"
                            id="BtnSubmitBulkApprove">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#TagihanTable').DataTable({
                ...
                columnDefs: [
                    @if (auth()->user()->role === 'Superadmin')
                        // Superadmin: 0:Check, 1:#, 2:No, 3:Tenant, 4:Periode, 5:JatuhTempo, 6:BerlakuHingga, 7:TglBayar, 8:Jumlah, 9:Status, 10:Bukti, 11:Aksi
                        {
                            className: 'text-center',
                            targets: [0, 1, 5, 6, 7, 9, 10, 11]
                        }, {
                            className: 'text-end',
                            targets: [8]
                        }, {
                    ...
                    @else
                        // Admin: 0:#, 1:No, 2:Tenant, 3:Periode, 4:JatuhTempo, 5:BerlakuHingga, 6:TglBayar, 7:Jumlah, 8:Status, 9:Bukti, 10:Aksi
                        {
                            className: 'text-center',
                ...
                columns: [
                    ...
                    {
                        data: 'BerlakuHingga',
                        name: 'BerlakuHingga', // Tambahan BerlakuHingga
                        render: function(Data) {
                            return Data ? new Date(Data).toLocaleDateString('id-ID', {
                                day: '2-digit',
                                month: 'short',
            ...
            $('body').on('click', '.btn-hapus', function() {
                ...
                Swal.fire({
                    ...
                    if (Result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('tagihan-pembayaran.destroy', ':id') }}"
                                .replace(
                                    ':id', Id),
                            type: 'POST',
                            data: {
                            ...
                            success: function(Response) {
                                if (Response.success) {
                                    ...
                                    $('#TagihanTable').DataTable().ajax.reload(null,
                                        false);
                                } else {
